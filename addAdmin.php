...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['addAdmin'])){
    $adminRegistration = $registration->adminRegistrations($_POST);
}

?> 

<section class ="container addStuff">
    <h5 style="color:#282C34;" class="addStuffInput">New Admin Form</h5>
    <hr>

    <?php
        if (isset($adminRegistration)) {
            echo $adminRegistration;
        }
    ...
    <form method="post" action="addStuff.php" class="row">
        ...
        <button name="addAdmin" class="w-25 mt-3 ms-2 btn btn-primary" type="submit">Add Admin</button>
    </form>
</section>
